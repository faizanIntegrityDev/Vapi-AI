...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (!tech || !experience || !level) {
    return res.status(400).json({ error: 'Missing required fields: tech, experience, or level' });
  }

  const context = `Generate a technical interview question for a candidate applying for a ${level} position in ${tech} with ${experience} years of experience.`;

  try {
    console.log(`[VAPI] Incoming tech: ${tech}, experience: ${experience}, level: ${level}`);
    
    const question = await askMetaAI(context);
    
    console.log(`[MetaAI] Generated question: ${question}`);
    
    return res.status(200).json({ meta_question: question });
  } catch (error) {
    console.error('[Meta AI Error]', error);
    return res.status(500).json({ error: 'Failed to generate question from Meta AI.' });
  }
}
